...
class RegistrationPointDataAquisition(object):
  ...
  def __init__(self, fixed_image, moving_image, fixed_window_level=None, moving_window_level=None, figure_size=(10, 8),
...
class PointDataAquisition(object):
  ...
  def validate_points(self, point_index_data):
    for p in point_index_data:
      if p[0] >= self.npa.shape[2] or p[0] < 0 or p[1] >= self.npa.shape[1] or p[1] < 0 or p[2] >= self.npa.shape[0] or \
          p[2] < 0:
...
class ROIDataAquisition(object):
  ...
  def validate_rois(self, roi_data):
    for roi in roi_data:
      ...
      if roi[0][0] >= self.npa.shape[2] or roi[0][1] < 0 or roi[1][0] >= self.npa.shape[1] or roi[1][1] < 0 or roi[2][
